function test_server_evidence
% Load-only checks for server evidence. This file must never invoke a solve.

repoRoot = fileparts(fileparts(fileparts(mfilename('fullpath'))));
resultRoot = fullfile(repoRoot, 'results', 'diagnostic');

k5 = load(fullfile(resultRoot, 'server_exact_d2_k5_saved_blocks.mat'));
...
k56 = load(fullfile(resultRoot, 'server_exact_k56_checkpoint.mat'));
...
comparison = load(fullfile(resultRoot, 'server_full_vs_linear_k1_k3.mat'));
...
fprintf('server evidence load-only checks passed.\n');
end
