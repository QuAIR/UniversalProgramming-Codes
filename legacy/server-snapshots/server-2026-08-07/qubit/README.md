...
- `run_server_exact_k14.m` runs only the corrected exact SDP for
  `k=1,2,3,4` on server.
- `run_server_exact_k56.m` runs the saved-block exact batch for `k=5,6`.
...
On server the working configuration is CVX with SDPT3:
