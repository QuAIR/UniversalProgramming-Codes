...
addpath(genpath("<QETLAB_ROOT>")); addpath(genpath("<YALMIP_ROOT>")); addpath("<MOSEK_INTERFACE>");
d=5; k=3; s=256;
...
for c=1:s
  ...
  Y=reshape(permute(R4x,[4 2 3 1]), dk^2, dk^2); clear temp Xt Xg R4x; if mod(c,8)==1, fprintf("  asm %d/%d" + newline, c, s); end
  for j1=1:bl1
    Z=M1{j1}*Y;
    for j2=1:bl2
      G=Z*M2{j2}.';
...
% reduced-space certificate (no D x D reconstruction)
y1 = NS'*b1; y2 = NS'*b2;
mineig = inf; maxnh = 0;
for ip = 1:nbp
  w = wsz(ip);
  Jb1 = reshape(Rmat{ip}*y1, w, w); Jb2 = reshape(Rmat{ip}*y2, w, w);
  mineig = min([mineig, min(real(eig((Jb1+Jb1')/2))), min(real(eig((Jb2+Jb2')/2)))]);
  maxnh = max([maxnh, norm(Jb1-Jb1',"fro"), norm(Jb2-Jb2',"fro")]);
end
tpres = max(norm(TPb*y1 - p1*tpv), norm(TPb*y2 - p2*tpv));
rng(777); nf = 12; fres = 0;
for c = 1:nf
  JCf = RandomSuperoperator(d)/d;
  temp = JCf; for kk = 1:k-1, temp = kron(temp, JCf); end
  Xt = temp.'; Xg = PermuteSystems(Xt, gperm, d*ones(1,2*k));
  R4x = reshape(Xg, [dk dk dk dk]); Y = reshape(permute(R4x, [4 2 3 1]), dk^2, dk^2); clear temp Xt Xg R4x;
  prog = zeros(d^2);
  for j1 = 1:bl1
    Z = M1{j1}*Y;
    for j2 = 1:bl2
      G = Z*M2{j2}.'; R4g = reshape(G, [d d d d]);
      Pj = reshape(permute(R4g, [3 1 4 2]), d^2, d^2);
      idx = (j1-1)*bl2 + j2; prog = prog + (b1(idx)-b2(idx))*Pj;
    end
  end
  clear Y;
  fres = max(fres, norm(prog - d*JCf, "fro"));
end
fprintf("CERT(reduced): min eig %.3e ; nonherm %.2e ; TP res %.2e ; fresh prog (%d ch) %.3e" + newline, mineig, maxnh, tpres, nf, fres);
if ~exist("results","dir"); mkdir("results"); end
save(sprintf("results/y3_d%d_k%d.mat",d,k),"cost","d","k","s","b1","b2","p1","p2","cvx_status","mineig","maxnh","tpres","fres");
fprintf("saved" + newline);
