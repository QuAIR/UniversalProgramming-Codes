...
s = 500;       % random channel samples for programming constraint
...
m = bl1 * bl2;
V1=zeros(Dsec^2,bl1); for jj=1:bl1, tmp=basis_1k{jj}; V1(:,jj)=tmp(:); end; V1i=pinv(V1); A1=zeros(bl1); for jj=1:bl1, tt=transpose(basis_1k{jj}); A1(:,jj)=real(V1i*tt(:)); end; V2=zeros(Dsec^2,bl2); for jj=1:bl2, tmp=basis_k1{jj}; V2(:,jj)=tmp(:); end; V2i=pinv(V2); A2=zeros(bl2); for jj=1:bl2, tt=transpose(basis_k1{jj}); A2(:,jj)=real(V2i*tt(:)); end; Aherm=kron(A1,A2);
...
Aherm*b1 == b1; Aherm*b2 == b2;
    ...
    TP1 == p1 * eye(dim_tp);  %#ok<EQEFF>
    TP2 == p2 * eye(dim_tp);  %#ok<EQEFF>

    % ---- (C3') Programming ----
    for c = 1:s
        prog = zeros(d^2);
        for j = 1:m
            prog = prog + (b1(j) - b2(j)) * P(:,:,j,c);
        end
        prog == d * JC(:,:,c);  %#ok<EQEFF>
    end
cvx_end

cost = p1 + p2;
if ~exist("results","dir"); mkdir("results"); end; save(sprintf("results/gamma_d%d_k%d_fix.mat",d,k),"cost","d","k","s","method","cvx_status");
