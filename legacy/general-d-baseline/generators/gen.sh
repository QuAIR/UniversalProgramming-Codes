...
sed_in_place "$OUT" "/^I_d/a repo_root = getenv(\"UP_REPO_ROOT\"); if isempty(repo_root), repo_root = \"$fallback_repo_root\"; end; addpath(fullfile(repo_root, \"src\", \"matlab\")); cfg = up_config(); up_setup(cfg, false); rng($SEED);"
sed_in_place "$OUT" "/^m = bl1/a V1=zeros(Dsec^2,bl1); for jj=1:bl1, tmp=basis_1k{jj}; V1(:,jj)=tmp(:); end; V1i=pinv(V1); A1=zeros(bl1); for jj=1:bl1, tt=transpose(basis_1k{jj}); A1(:,jj)=real(V1i*tt(:)); end; V2=zeros(Dsec^2,bl2); for jj=1:bl2, tmp=basis_k1{jj}; V2(:,jj)=tmp(:); end; V2i=pinv(V2); A2=zeros(bl2); for jj=1:bl2, tt=transpose(basis_k1{jj}); A2(:,jj)=real(V2i*tt(:)); end; Aherm=kron(A1,A2); AhermC=Aherm-eye(bl1*bl2); AhermC(abs(AhermC)<1e-9)=0;"
sed_in_place "$OUT" "/^perm(2\\*k+2) = 2\\*k+2;/a ipm = zeros(1,2*k+2); ipm(perm) = 1:(2*k+2); perm = ipm;"
# pre-reduce all equality families to full rank (CVX 2.2 eliminate chokes on redundant rows)
sed_in_place "$OUT" "/^cvx_begin sdp quiet/i TPR = orth([reshape(T,[],m), -reshape(eye(dim_tp),[],1)]')'; APR=zeros(2*d^4*s,m); BPR=zeros(2*d^4*s,1); for c=1:s, Mc=reshape(P(:,:,:,c),d^4,m); rh=d*reshape(JC(:,:,c),d^4,1); APR((c-1)*2*d^4+(1:d^4),:)=real(Mc); BPR((c-1)*2*d^4+(1:d^4))=real(rh); APR((c-1)*2*d^4+d^4+(1:d^4),:)=imag(Mc); BPR((c-1)*2*d^4+d^4+(1:d^4))=imag(rh); end; RowB=orth([APR BPR]')'; ARED=RowB(:,1:end-1); BRED=RowB(:,end); AHR=orth(AhermC')'; clear APR BPR RowB;"
sed_in_place "$OUT" "/^cvx_begin sdp quiet/i cvx_solver mosek;"
sed_in_place "$OUT" "/minimize(p1 + p2)/a if ~isempty(AHR); AHR*b1 == 0; AHR*b2 == 0; end; ARED*(b1-b2) == BRED; TPR*[b1;p1] == 0; TPR*[b2;p2] == 0;"
...
sed_in_place "$OUT" "/^cost = p1 + p2;/a outputDir = fullfile(cfg.resultsRoot, \"legacy\", \"general-d-baseline\"); if exist(outputDir, \"dir\") ~= 7, mkdir(outputDir); end; save(fullfile(outputDir, sprintf(\"gamma_d%d_k%d_s%d_r%d.mat\",d,k,s,$SEED)),\"cost\",\"d\",\"k\",\"s\",\"method\",\"cvx_status\");"
printf '%s\n' "$OUT"
