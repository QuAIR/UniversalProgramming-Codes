...
for c = 1:nf
  ...
  R4x = reshape(Xg, [dk dk dk dk]); Y = reshape(permute(R4x, [4 2 3 1]), dk^2, dk^2);
  prog = zeros(d^2);
  for j1 = 1:bl1
    Z = M1{j1}*Y;
    for j2 = 1:bl2
...
save(fullfile(outputDir, sprintf("struct3_d%d_k%d.mat",d,k)),"cost","d","k","s","b1","b2","p1","p2","cvx_status","mineig","maxnh","tpres","fres");
