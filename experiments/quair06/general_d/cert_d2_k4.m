...
cfg = up_config();
up_setup(cfg, false);
outputDir = fullfile(cfg.resultsRoot, 'quair06', 'general_d', 'certificates');
if exist(outputDir, 'dir') ~= 7; mkdir(outputDir); end
d=2; k=4; s=500; n=k+1; Dsec=d^n; D=d^(2*k+2); dk=d^k; dim_tp=d^(2*k+1); I_d=eye(d);
rng(0);
...
outputFile = fullfile(outputDir, 'cert_d2_k4.mat');
