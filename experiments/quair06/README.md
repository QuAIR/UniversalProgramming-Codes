...
inconsistent saved result causes an error. No script automatically deletes,
overwrites as cleanup, or truncates previous result directories. Use a new
`UP_RESULTS_ROOT` to preserve an earlier run unchanged.
...
[`general_d/README.md`](general_d/README.md). Run one from the repository root:

```sh
matlab -batch "run('experiments/quair06/general_d/cert_d2_k3.m')"
matlab -batch "run('experiments/quair06/general_d/gamma_struct3.m')"
matlab -batch "run('experiments/quair06/general_d/gamma_y3.m')"
```
...
those historical choices. Several general-`d` files retain hard-coded
historical dimensions and copy counts. Review the entry and choose a distinct
`UP_RESULTS_ROOT` before any parameter change. Diagnostic entries remain
diagnostic after rerunning.
