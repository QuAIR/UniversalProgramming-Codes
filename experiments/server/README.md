...
./experiments/server/kcopy_d2/run_exact_k56.sh
```
...
`$UP_RESULTS_ROOT/server/kcopy_d2/launch/`. They do not inspect, submit to, or
terminate an external scheduler.

From an interactive MATLAB session, run either batch directly:

```matlab
run('experiments/server/kcopy_d2/run_exact_k14.m')
run('experiments/server/kcopy_d2/run_exact_k56.m')
```
...
```sh
matlab -batch "run('experiments/server/general_d/cert_d2_k3.m')"
```
...
`$UP_RESULTS_ROOT/server/general_d/{certificates,validated,diagnostics}`.

## Read-only reconciliation

On 2026-08-07, the two live server trees were inventoried read-only and
...
[`docs/provenance/server-live-inventory.json`](../../docs/provenance/server-live-inventory.json).
