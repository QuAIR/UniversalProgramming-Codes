...
1. the research-code tree removed from the manuscript repository after commit
   `4512790`;
2. files and result artifacts previously downloaded from quair06;
3. execution records that contain recoverable server-only diagnostic scripts;
   and
4. the current quair06 working directory, when the server is reachable.
...
`experiments/quair06/` contains scripts that define actual server runs. The
...
original source commit or quair06 path where available.
...
When quair06 is reachable, its `~/projects/kcopy_d2` tree is inventoried by
